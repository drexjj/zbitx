...
void cmd_exec(char *cmd)
{
	...
	{
		// Sent by the Pico W front panel after the user has already
		// confirmed on the touchscreen. Skip the GTK dialogs and
		// shut down immediately using the same command as PWR-DWN.
		tx_off();
		save_user_settings(1);
		system("sudo /sbin/shutdown -h now");
	}
